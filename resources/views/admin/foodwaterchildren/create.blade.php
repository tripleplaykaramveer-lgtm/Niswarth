            ...
            <div class="card border-top border-0 border-4 border-white">
                <div class="card-body">
                    <div class="border p-4 rounded">
                        ...
                        @endif
                        {{-- Form Start --}}
                        <form action="{{ route('foodwater.children.store') }}" method="POST">
                            @csrf
                            <div class="row">

                                {{-- Food & Water Category --}}
                                <div class="col-md-6 mb-3">
                                    <label for="food_water_subcategory_id" class="form-label">Food & Water Sub
                                        Category</label>
                                    <select class="form-select" id="food_water_subcategory_id"
                                        name="food_water_subcategory_id" required>
                                        <option value="">Select</option>
                                        @foreach ($foodwaterchild as $id => $name)
                                            <option value="{{ $id }}"
                                                {{ old('food_water_subcategory_id') == $id ? 'selected' : '' }}>
                                                {{ $name }}
                                            </option>
                                        @endforeach
                                    </select>
