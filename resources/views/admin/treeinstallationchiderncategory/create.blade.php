        ...
        <div class="card border-top border-0 border-4 border-white">
            <div class="card-body">
                <div class="border p-4 rounded">
                    ...
                    {{-- Form Start --}}
                    <form action="{{ route('treeinstallationchaildrencategory.store') }}" method="POST">
                        @csrf
                        <div class="row">
                           <div class="col-md-6 mb-3">
                                <label for="tree_installation_subcategory_id" class="form-label">Spiritual subcotegories Name</label>
                                <select class="form-select" id="tree_installation_subcategory_id" name="tree_installation_subcategory_id">
                                    ...
                                    @if (isset($treeinstallationsubcategorys) && $treeinstallationsubcategorys->isNotEmpty())
                                        @foreach ($treeinstallationsubcategorys as $id => $treeinstallationsubcategory)
                                            <option value="{{ $id }}"
                                                {{ old('tree_installation_subcategory_id') == $id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="tree-chidern-category" class="form-label">spiritual chaildern categories</label>
                                <select class="form-control" name="tree_installation_chidern_category[]" id="tree-chidern-category" multiple>
                                </select>
