...
function renderStep(step, title, items, type, context) {
    let html = `<h5>Step ${step}: ${title}</h5>`;
    html += `<div class="custom-button-group d-flex flex-wrap">`;

    items.forEach((item, index) => {
        html += `
                <button class="custom-select-btn next-step ${index === 0 ? 'selected' : ''}"
                        data-id="${item.id}"
                        data-type="${type}"
                        data-step="${step}"
    ...
    $("#modalBodyContent").append(`<div class="step-block" id="${context}-step-${step}">${html}</div>`);
    updateStepper(step, title, context);
}


function updateStepper(step, title, context) {
    let $stepper = $("#stepperSteps");

    $stepper.find(".stepper-step").removeClass("active");

    if ($(`#${context}-stepper-step-${step}`).length === 0) {
        $stepper.append(`<div id="${context}-stepper-step-${step}" class="stepper-step">Step ${step}: ${title}</div>`);
    }

    $(`#${context}-stepper-step-${step}`).addClass("active");

    let totalSteps = $(".step-block").length;
...
$(document).on("click", ".next-step", function () {
    ...
    if (type === "subcategory") {
        $.get(`/${context}-children/${id}`, function (data) {
    ...
    if (type === "child") {
        $.get(`/${context}-minichildren/${id}`, function (data) {
            if (Array.isArray(data) && data.length > 0) {
...
$(document).on("click", ".custom-select-btn", function () {
    ...
    $(`#${context}-step-${step} .custom-select-btn`).removeClass("selected");

    $(this).addClass("selected");
});


// Clears the model on cancel
